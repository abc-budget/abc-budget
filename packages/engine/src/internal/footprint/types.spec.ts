 ...
 * VIS-002) and holds EXACTLY 5 fields. This spec enforces that at COMPILE TIME:
 * a type-level exact-key guard so that adding a 6th field FAILS THE BUILD (the
...
type ExactShape = { year: 0; month: 0; amountUSD: 0; categoryId: 0; hash: 0 };

// COMPILE-TIME exact-key guard: flips to `false` (build error) if FootprintRecord
// gains or loses a key relative to ExactShape. Adding a 6th field fails here.
const _exact: KeysEqual<FootprintRecord, ExactShape> = true;
void _exact;

describe('FootprintRecord — minimization contract (ENT-001)', () => {
  it('has EXACTLY the 5 allowed keys at the type level', () => {
    expectTypeOf<keyof FootprintRecord>().toEqualTypeOf<
      'year' | 'month' | 'amountUSD' | 'categoryId' | 'hash'
    >();
  });

  it('a valid record literal has exactly 5 keys at runtime', () => {
