import type { ChromeKey } from './catalog-uk';

/** Chrome strings — en. `satisfies` makes key drift a compile error. */
export const CATALOG_EN = {
  langToggleLabel: 'UI language',
  zoneDashboard: 'Dashboard',
  zoneSettings: 'Settings',
  stepFile: 'FILE',
  stepColumns: 'COLUMNS',
  stepCategories: 'CATEGORIES',
  stepReview: 'REVIEW',
  stepOfTotal: 'STEP {n} / {total}',
  obTitle: 'Welcome',
  obLead: 'First run. Import a statement to get started.',
  ctaImportStatement: 'Import a statement',
  ctaTryExample: 'Try an example',
  dashTitle: 'Dashboard',
  dashLead: 'Your budget will appear here (EP-6). For now — import a statement.',
  ctaImport: 'Import statement',
  setTabOverview: 'Overview',
  setTabCategories: 'Categories',
  setDataTitle: 'Data',
  setDataLead: 'Base currency, language, threshold — in upcoming stories.',
  setCatTitle: 'Categories',
  setCatLead: 'Category management — EP-5.',
  impSourceTitle: 'Source',
  impSourceNote: 'File upload — EP-2.1 (the wedge).',
  impColumnsTitle: 'Columns',
  impColumnsNote: 'Column mapping + UNKNOWN-gate — EP-2.',
  impCategoriesTitle: 'Categorization',
  impCategoriesNote: 'Rules, RUL/, LOG/ — EP-4.',
  impReviewTitle: 'Review & save',
  impReviewNote: 'Saving the footprint — EP-3.',
  keyBack: 'Back',
  keyNext: 'Next',
  keyImportMore: 'Import another',
  keyToBudget: 'To the budget',
  engBlockedTitle: 'Storage blocked',
  engBlockedBody: 'Close other ABC Budget tabs and reload the page.',
  engMismatchTitle: 'Update required',
  engMismatchBody: 'App versions are out of sync. Reload to update.',
  engMismatchReload: 'Reload',
  engDiedTitle: 'Engine restarting',
  engDiedBody: 'The background engine stopped — the current operation was interrupted. It will respawn automatically.',
  // ── S3a — Import → Source (2.7; ported from design-reference/s3a-i18n.jsx) ──
  s3aEyebrow: '▸ IMPORT · SOURCE',
  s3aStepOf: 'STEP 1 / 4',
  s3aTitle: 'Choose a statement file',
  s3aLead: 'Take the statement you exported from your bank. ABC reads it here, on this device — nothing is uploaded to the internet.',
  // drop zone
  s3aDropTitle: 'Drop the file here',
  s3aDropOr: 'or',
  s3aPick: 'Choose file',
  s3aFormats: 'CSV · XLS · XLSX · up to 50 MB',
  s3aLocalOnly: 'LOCAL · THE FILE NEVER LEAVES THIS DEVICE',
  s3aSample: 'Try a sample',
  // file chip
  s3aReplace: 'Replace',
  s3aRemove: 'Remove',
  s3aRowsEst: 'rows (approx.)',
  // recognized (per-column recall from the learned pool — NOT a "format")
  // prototype recogTitle(n,m) is conditional → two parameterized keys; the component picks on n===m
  s3aRecogTag: '▸ RECALLED FROM YOUR RULES',
  s3aRecogTitleAll: 'All {m} columns recognized',
  s3aRecogTitleSome: 'Recognized {n} of {m} columns',
  s3aRecogBody: 'These column names match ones you mapped before, so ABC filled in their types from your saved rules. This is an exact name match — not a guess, not AI.',
  s3aRecogPartial: '{k} still untyped — you’ll map them on the next step.',
  s3aPoolLab: 'FILE COLUMNS → YOUR RULES',
  s3aRecalled: 'from rules',
  s3aUnkType: 'untyped',
  s3aDedupTitle: 'Repeat transactions aren’t counted twice',
  s3aDedupBody: 'If the file contains transactions you imported before, ABC recognizes and merges them. Exact counts appear on the Review step.',
  s3aProceedNote: 'Every import passes through validation. The full parse runs when you press “Continue”.',
  s3aReviewMap: 'Review mapping',
  s3aProceed: 'Continue ▸',
  // unknown = empty pool (first-ever import) → all columns untyped
  s3aUnkTag: '▸ NO KNOWN COLUMN',
  s3aUnkTitle: 'First import — no rules yet',
  s3aUnkBody: 'ABC hasn’t seen these column names before, so they’re all untyped for now. On the next step you’ll map them by hand, and ABC will remember the names — next time it fills the types in automatically.',
  s3aUnkAllCols: '{m} columns · all untyped',
  s3aNextUnk: 'Next · map columns ▸',
  // error (fail loud, HC-7). The *V values are DEFAULTS for the generic
  // read-failure case — the real ErrorPanel renders decode-issue-driven props;
  // the container falls back to these when no specific issue copy exists.
  s3aErrTag: '▸ COULDN’T READ THE FILE',
  s3aErrWhat: 'WHAT:',
  s3aErrWhy: 'WHY:',
  s3aErrDo: 'DO:',
  s3aErrWhatV: 'The file could not be opened',
  s3aErrWhyV: 'It’s empty, corrupted, or not a spreadsheet (CSV/XLS/XLSX).',
  s3aErrDoV: 'Make sure it’s a statement export and try another file.',
  // issue-specific ЧОМУ (2.7 Task 3 — derived from the FATAL DecodeIssue kind)
  s3aErrWhyNoData: 'No data rows were found in the file.',
  s3aErrWhyUnreadable: 'The content could not be parsed — the file is corrupted or not a CSV/XLS/XLSX.',
  s3aTryAgain: 'Choose another file',
  s3aBack: 'Back',
  // decoding (DecodingPanel — NO bundle equivalent; dev-designed copy, PM pixel pass)
  s3aDecodingTag: '▸ READING THE FILE',
  s3aDecodingPrep: 'OPENING THE FILE…',
  s3aDecodingRows: '{done} / {total} rows',
  // base-currency cold-start dialog (Task 4 consumes; keys ported with the bundle set)
  s3aBaseTitle: 'Base currency',
  s3aBaseBody: 'Overall budget totals are computed in one currency. We detected it automatically — confirm it before your first import.',
  s3aBaseLabel: 'Currency',
  s3aBaseAuto: 'DETECTED BY REGION · YOU CAN CHANGE IT LATER IN SETTINGS',
  s3aCancel: 'Cancel',
  s3aCont: 'Continue ▸',
  // dialog optgroup labels + the loud save-failure line (Task 4) — NO bundle
  // source (the prototype select had only the curated 8): dev-designed copy,
  // PM eyeballs at the pixel pass.
  s3aBaseGroupCurated: 'Common',
  s3aBaseGroupAll: 'All currencies',
  s3aBaseError: 'Couldn’t save the currency',
  // multi-sheet neutral note (2.1 decision — flagged, not invented)
  s3aOtherSheets: 'The file has other sheets: {names} — only the first one was read.',
  // useBlocker exit-protection (1.5 carry-forward, lands at 2.7 Task 3)
  s3aLeaveTitle: 'Abandon this import?',
  s3aLeaveBody: 'If you leave now, the current import is cancelled. You will need to load the file again.',
  s3aLeaveStay: 'Stay',
  s3aLeaveLeave: 'Abandon & leave',
  // ── S3b — Import → Columns (2.8; ported from design-reference/s3b-data.jsx `M`) ──
  s3bEyebrow: '▸ IMPORT · COLUMNS',
  s3bStepOf: 'STEP 2 / 4',
  s3bTitle: 'Map the columns',
  s3bLead: 'ABC filled in column types you mapped before (from your rules) — confirm or fix. It’s an exact name match, not AI.',
  // raw statement header
  s3bRaw: 'RAW STATEMENT',
  s3bTransient: 'TRANSIENT VIEW · NOT STORED',
  s3bRows: 'rows',
  s3bCols: 'columns',
  // column states
  s3bUnknown: 'unknown',
  s3bUnknownShort: 'no type',
  s3bGuessed: 'from rules',
  s3bGuessedN: 'from rules',
  s3bConfirmed: 'set',
  s3bIgnored: 'ignored',
  // status panel
  s3bStatusTitle: 'MAPPING STATUS',
  s3bRecallNote: '"From rules" = ABC filled the type by an exact column-name match against your saved rules. Deterministic recall, not AI. First import (no rules yet) → every column untyped.',
  // normalization panel
  s3bNormTitle: 'CURRENCY NORMALIZATION',
  s3bNormSub: 'symbol · code → ISO',
  // column menu actions
  s3bPickType: 'Pick a column type',
  s3bMore: 'More… (configure)',
  s3bConfirm: 'Confirm',
  s3bReconfigure: 'Reconfigure',
  s3bUndo: 'Undo (revert)',
  // collision (FEAT-005 — recalled name whose saved params differ; LOUD, non-blocking)
  s3bCollTag: '▸ SAVED RULE DIFFERS',
  s3bCollBody: 'The saved rule’s parameters for “{col}” differ from the current ones. Confirm to update the rule, or keep it as is.',
  s3bCollColBadge: 'saved rule’s params differ · confirm/adjust',
  s3bCollConfirm: 'Update the rule',
  s3bCollDecline: 'Keep saved',
  // block panel (UNKNOWN gate — Option A)
  s3bBlockTag: '▸ COLUMNS WITHOUT A TYPE',
  s3bBlockBody: 'You can’t continue until every column has a type. Assign a type or mark “Ignore” for:',
  s3bBlockFix: 'Go to the first',
  // config wizard
  s3bCfgStep1: 'STEP 1 · TYPE',
  s3bCfgStep2: 'STEP 2 · PARAMETERS',
  s3bCfgFor: 'Configure column',
  s3bCfgApply: 'Apply',
  s3bCfgCancel: 'Cancel',
  s3bCfgBack: 'Back',
  s3bCfgPreview: 'PREVIEW',
  // worker progress panel (large file background processing)
  s3bWorkerTag: '▸ LARGE FILE · BACKGROUND PROCESSING',
  s3bWorkerTitle: 'Parsing the statement',
  s3bWorkerBody: 'The file is large, so processing runs in the background — the app stays responsive. You can watch the progress.',
  s3bWorkerRows: 'rows processed',
  s3bWorkerHint: '▸ 1–2 MONTHS PARSE INSTANTLY · THIS ONE IS BIGGER',
  // parse-error / rejection panel (WHAT/WHY/DO)
  s3bPerrTag: '▸ PARSE ERROR IN COLUMN',
  s3bPerrWhat: 'WHAT:',
  s3bPerrWhy: 'WHY:',
  s3bPerrDo: 'DO:',
  s3bPerrReview: 'Review problem rows',
  // rejection panel value lines (>30% ColumnTransformRejection)
  s3bPerrTagCol: '▸ PARSE ERROR IN COLUMN · {col}',
  s3bRejWhat: '{errors} of {total} values could not be parsed',
  s3bRejWhy: 'That exceeds the {pct}% error threshold, so the whole column was rolled back.',
  s3bRejDo: 'Pick a different type, fix the format, or mark the column to ignore.',
  s3bRejRow: 'ROW {row}',
  // navigation
  s3bNext: 'Next ▸',
  s3bBack: 'Back',
  s3bFoot: '▸ LOCAL · OFFLINE · RAW ROWS ARE TRANSIENT — NOT STORED',
  // help panel
  s3bHelpIntro: 'ABOUT THIS STEP',
  s3bSelectColHint: 'Select a column to see help for its type.',
  // parameterized: showing(n, m) and ofFull(n, m)
  // 1.4 pattern: template strings with {n} / {m} placeholders
  s3bShowing: 'showing {n} of {m}',
  s3bOfFull: '{n} of {m} in file',

  // ── S3c — Import → Categorization (4.9a; ported from design-reference/s3c-i18n.jsx `C`) ──
  s3cEyebrow: '▸ IMPORT · CATEGORIZATION',
  s3cStepOf: 'STEP 3 / 4',
  s3cTitle: 'Sort operations into categories',
  s3cLead:
    'Spot a familiar operation — filter the similar ones and assign them all at once. That creates a transparent rule, not "magic". The same file always gives the same result.',
  s3cOpsTitle: 'OPERATIONS',
  s3cColDate: 'Date',
  s3cColDesc: 'Operation',
  s3cColAmount: 'Amount',
  s3cColCat: 'Category',
  s3cColMcc: 'MCC',
  s3cColCurrency: 'Currency',
  s3cColAccount: 'account',
  s3cColCounterparty: 'counterparty',
  s3cSegAll: 'All',
  s3cSegUncat: 'Uncategorized',
  s3cOpsTotal: '{n} ops',
  s3cPageOf: '{n} / {m}',
  s3cAssignManual: 'Assign',
  s3cWhyTitle: 'Why this category?',
  s3cOpsEmpty: 'no operations match',
  s3cFilterLabel: 'FILTER',
  s3cClearFilter: 'Clear filter',
  s3cFunnelTitle: 'Filter by this column',
  s3cPrev: 'Previous',
  s3cNextPage: 'Next',
  s3cRulesTitle: 'RULES EDITOR',
  s3cTabBuild: 'Filter / rule',
  s3cTabRules: 'All rules',
  s3cRulesSearch: 'Search rules…',
  s3cFirstMatch: 'FIRST MATCH WINS · TOP TO BOTTOM',
  s3cDraftEmpty:
    'The filter builder is empty. Click a column funnel or “Add condition” — the list narrows to matches.',
  s3cLiveMatch: '{n} matches in list',
  s3cSaveAsRule: 'Save as rule',
  s3cPickCat: 'Category for the rule',
  s3cClearDraft: 'Clear',
  s3cCreateRuleNote: '▸ This is NOT a one-off — it is a RULE. Visible and editable in the rules editor.',
  s3cApplied: '{n} ops',
  s3cRuleArrow: '→',
  s3cRuleRest: 'rest',
  s3cEmptyTag: '▸ NOTHING CATEGORIZED YET',
  s3cBldAdd: 'Add condition',
  s3cBldAnd: 'AND',
  s3cBldRegexPh: '^PATTERN$',
  s3cBldCurPairNote: 'Currency is added automatically — comparing amount without a currency is meaningless.',
  s3cBldCurLab: 'Currency',
  s3cBldRemove: 'remove',
  s3cFieldDate: 'date',
  s3cFieldAmount: 'amount',
  s3cFieldCur: 'currency',
  s3cFieldMcc: 'MCC',
  s3cFieldCategory: 'bank category',
  s3cFieldDesc: 'description',
  s3cFieldAccount: 'account',
  s3cFieldCounterparty: 'counterparty',
  s3cOpEq: '=',
  s3cOpNeq: '≠',
  s3cOpContains: 'contains',
  s3cOpNcontains: 'doesn’t contain',
  s3cOpStarts: 'starts with',
  s3cOpEnds: 'ends with',
  s3cOpMatches: 'regex',
  s3cOpGt: '>',
  s3cOpLt: '<',
  s3cOpGte: '≥',
  s3cOpLte: '≤',
  s3cOpBetween: 'between',
  s3cOpOneof: 'one of',
  s3cOpIsTrue: 'yes',
  s3cOpIsFalse: 'no',
  s3cOpSpecificDay: 'day',
  s3cOpDayRange: 'day range',
  s3cOpFirstDayOfMonth: 'first day of month',
  s3cOpFirstMondayOfMonth: 'first Monday of month',
  s3cOpFirstSaturdayOfMonth: 'first Saturday of month',
  s3cOpFirstSundayOfMonth: 'first Sunday of month',
  s3cOpLastDayOfMonth: 'last day of month',
  s3cOpLastMondayOfMonth: 'last Monday of month',
  s3cOpLastSaturdayOfMonth: 'last Saturday of month',
  s3cOpLastSundayOfMonth: 'last Sunday of month',
  s3cWhyEvalLab: '// RULE EVALUATION · TOP TO BOTTOM · FIRST MATCH WINS',
  s3cWhyEvalLabManual: '// RULES NOT EVALUATED · MANUAL OVERRIDE WINS',
  s3cWhyWinner: 'WINNING RULE',
  s3cWhyMiss: 'not met',
  s3cWhyNeutral: 'not evaluated · short-circuit',
  s3cWhyManual: 'SET MANUALLY · STICKY',
  s3cWhyNoRule: 'No rule fired — the operation is uncategorized.',
  s3cWhyOp: 'OPERATION',
  s3cClose: 'Close',
  s3cOverrideOn: 'MANUAL',
  s3cPickSearch: 'Search categories…',
  s3cPickNone: 'nothing found',
  s3cCreateCat: 'Create category',
  s3cCreateNamed: 'Create “{q}”',
  s3cNewCatTitle: 'New category',
  s3cFName: 'Name',
  s3cFNamePh: 'e.g. Groceries',
  s3cFIcon: 'Icon',
  s3cFCur: 'Category currency',
  s3cIconSearchPh: 'Search icon…',
  s3cIconNone: 'no icons found',
  s3cFollowBaseNote: 'Category follows the base currency',
  s3cBaseAlias: 'base',
  s3cCreate: 'Create',
  s3cCancel: 'Cancel',
  // ── S3c — 4.9b sandbox / rule-editing chrome ──
  s3cSbTag: '▸ VIRTUAL RULE TREE · UNSAVED',
  s3cSbCount: (n: number): string =>
    n === 0 ? 'no categories change' : `${n} operation${n === 1 ? '' : 's'} will change category`,
  s3cSbReview: 'Review changes',
  s3cSbReviewOff: 'All operations',
  s3cSbApply: 'Apply',
  s3cSbDiscard: 'Cancel',
  s3cUpdateRule: 'Update rule',
  s3cReviewEdit: 'Review changes ▸',
  s3cEdit: 'Edit',
  s3cDelete: 'Delete',
  s3cDragHint: 'Drag to reorder',
  s3cMoveUp: 'Up',
  s3cMoveDown: 'Down',
  // ── S3c — 4.9c completion gate + Auto-Other + self-check ──
  s3cGateOpenTag: '▸ ALL CATEGORIZED',
  s3cGateBlockedTag: '▸ NOT ALL CATEGORIZED',
  s3cGateOpen: 'All operations have a category. You can proceed to review.',
  s3cGateBlocked: (n: number): string => `${n} operation${n === 1 ? '' : 's'} still uncategorized.`,
  s3cAutoOther: 'Assign the rest…',
  s3cAoTitle: 'Assign the remaining operations',
  s3cAoOf: (n: number): string => `of ${n} uncategorized operation${n === 1 ? '' : 's'}`,
  s3cAoSumLab: 'Remainder sum · base currency',
  s3cAoApproxNote: 'approximate · rates may be refined on save',
  s3cAoRatePending: 'rate pending',
  s3cAoPick: 'Category for the rest',
  s3cAoTransient: 'This import only · not a rule, not a sticky assignment',
  s3cScEye: '▸ SET SELF-CHECK',
  s3cScBody: (n: number): string =>
    `${n} operation${n === 1 ? ' is the least typical' : 's are the least typical'} in this set — worth a look.`,
  s3cScSort: 'Atypical — on top',
  s3cScMicro: '// a hint, not a verdict — you decide',
  s3cScHide: 'Hide hint',
  s3cScTag: 'atypical',
  s3cRsnMccTitle: 'MCC ≠ SET',
  s3cRsnCat: '≠ SET',
  s3cRsnAmt: 'AMOUNT — OUTLIER',
  s3cRsnTxt: 'RARE WORDS',

  // ── S3d — Import → Review & save (5.4; ported from design-reference/s3d-i18n.jsx `D.en`) ──
  s3dEyebrow: '▸ IMPORT · REVIEW',
  s3dStepOf: 'STEP 4 / 4',
  s3dTitle: 'Review & save',
  s3dLead: 'Verify the parsed and categorized result, then save. Nothing is dropped silently — every skipped or errored row is explained.',
  // validation summary
  s3dSumTitle: 'SUMMARY',
  s3dSumOk: 'ready',
  s3dSumError: 'errors',
  s3dSumSkipped: 'skipped',
  s3dSumDup: 'duplicates',
  s3dSumNew: 'new to save',
  s3dOkTag: 'All clean — no errors',
  s3dErrTag: 'Errors present — review before saving',
  // dedup
  s3dDedupTitle: 'DE-DUPLICATION',
  s3dDedupNew: 'new',
  s3dDedupDup: 'already imported',
  s3dDedupNote: 'Duplicates are detected by operation footprint and skipped — re-importing never double-counts.',
  s3dNoDedup: 'No overlap with previous imports detected.',
  // show-rows filter
  s3dShowRows: 'SHOW ROWS',
  s3dFAll: 'All',
  s3dFError: 'Only errors',
  s3dFSkip: 'Only skipped',
  s3dFBoth: 'Errors & skipped',
  s3dShowing: 'showing {n} of {m}',
  // table
  s3dColState: 'STATE',
  s3dColDate: 'Date',
  s3dColDesc: 'Description',
  s3dColAmount: 'Amount',
  s3dColCat: 'Category',
  s3dStOk: 'ok',
  s3dStError: 'error',
  s3dStSkipped: 'skipped',
  s3dCatNone: '—',
  // fail-loud / privacy
  s3dReasonLab: '// REASON',
  s3dPrivacyNote: '▸ A MINIMIZED FOOTPRINT IS STORED (FULL DATE · no time-of-day · AMOUNT IN USD · CATEGORY · SHA256 OF ORIGINAL) + RULES · RAW STATEMENT DATA IS NOT PERSISTED',
  // footer / save
  s3dSave: 'Save import',
  s3dSaveCount: 'Save {n} operations',
  s3dAckErrors: 'Acknowledge errors',
  s3dErrBlock: '{n} errored rows won’t be saved',
  s3dSaving: 'Saving…',
  s3dSavedTitle: 'Import saved',
  s3dSavedBody: '{n} operations added to your budget. Raw statement data was discarded — only footprints and rules remain.',
  s3dSavedGoto: 'To budget ▸',
  s3dSavedAnother: 'Import another',
  s3dAddressErrors: 'Review the errored rows first (the “Only errors” filter). They will be skipped on save — confirm that’s expected.',
} satisfies Record<ChromeKey, string | ((n: number) => string)>;
